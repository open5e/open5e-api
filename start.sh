...
if [ -f /usr/share/elasticsearch/bin/elasticsearch ]; then
    ...
    echo "Elasticsearch not found in container, expecting external service"
fi
...
wait_for_elasticsearch() {
        ...
        if curl -f -s "$ES_HOST/_cluster/health?wait_for_status=yellow&timeout=5s" > /dev/null 2>&1; then
            ...
            if curl -f -s -X PUT "$ES_HOST/test-connection-index" > /dev/null 2>&1 && \
               curl -f -s -X DELETE "$ES_HOST/test-connection-index" > /dev/null 2>&1; then
                echo "Elasticsearch is ready for indexing"
                return 0
            else
    ...
    echo "Note: Vector search will be disabled, but text and fuzzy search will work"
...
if [ "${ELASTICSEARCH_AVAILABLE:-true}" = "false" ]; then
    echo "Running quicksetup in SQLite FTS mode (Elasticsearch not available)..."
    pipenv run python manage.py quicksetup --clean || {
        echo "Quicksetup had issues, but continuing with basic setup..."
